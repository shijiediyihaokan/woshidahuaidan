...
window.AdminEditor = (function() {
  ...
  function addModule(type) {
    ...
    __vizData.push(module);
    renderAll();
    scrollToViz(__vizData.length - 1);
  }
  ...
  function deleteModule(idx) {
    __vizData.splice(idx, 1);
    renderAll();
  }

  function moveUp(idx) {
    if (idx > 0) { var t = ____vizData[idx]; ____vizData[idx] = __vizData[idx - 1]; __vizData[idx - 1] = t; renderAll(); }
  }

  function moveDown(idx) {
    if (idx < __vizData.length - 1) { var t = ____vizData[idx]; ____vizData[idx] = __vizData[idx + 1]; __vizData[idx + 1] = t; renderAll(); }
  }
  ...
  function renderAll() {
    ...
    for (var i = 0; i < __vizData.length; i++) {
      html += renderModule(____vizData[i], i);
    }
    ...
    switch(m.type) {
      case 'text':
        h = '<div class="viz-module" id="viz-' + idx + '">' + actions +
          '<textarea oninput="AdminEditor.__vizData[' + idx + '].data.text=this.value" style="width:100%;min-height:80px;padding:12px;border:1px solid #e5e7eb;border-radius:6px;font-size:14px;line-height:1.8;resize:vertical;color:#444" placeholder="Enter content...">' + m.data.text + '</textarea></div>';
        break;
      case 'h2':
        h = '<div class="viz-module" id="viz-' + idx + '">' + actions +
          '<div style="display:flex;gap:8px;align-items:center">' +
          '<input value="' + m.data.text + '" placeholder="H2 Title" onchange="AdminEditor.__vizData[' + idx + '].data.text=this.value" style="flex:1;padding:8px 12px;border-left:4px solid ' + m.data.color + ';font-size:22px;font-weight:bold;background:#fafafa">' +
          '<select onchange="AdminEditor.__vizData[' + idx + '].data.style=this.value;AdminEditor.renderAll()" style="padding:4px;font-size:11px">' +
          ...
          '</select>' +
          '<input type="color" value="' + m.data.color + '" onchange="AdminEditor.__vizData[' + idx + '].data.color=this.value;AdminEditor.renderAll()" style="width:36px;height:28px">' +
          '</div></div>';
        break;
      case 'h3':
        h = '<div class="viz-module" id="viz-' + idx + '">' + actions +
          '<div style="display:flex;gap:8px;align-items:center">' +
          '<input value="' + m.data.text + '" placeholder="H3 Title" onchange="AdminEditor.__vizData[' + idx + '].data.text=this.value" style="flex:1;padding:6px 10px;border-left:4px solid ' + m.data.color + ';font-size:17px;font-weight:bold;background:#fafafa">' +
          '<select onchange="AdminEditor.__vizData[' + idx + '].data.style=this.value;AdminEditor.renderAll()" style="padding:4px;font-size:11px">' +
          '<option value="red-left"' + (m.data.style === 'red-left' ? ' selected' : '') + '>Red Left</option>' +
          '<option value="red-underline"' + (m.data.style === 'red-underline' ? ' selected' : '') + '>Red Underline</option>' +
          '</select>' +
      ...
      case 'image':
        ...
        h = '<div class="viz-module" id="viz-' + idx + '" style="padding:12px;border:1px dashed #e5e7eb;border-radius:8px;background:#fafafa">' + actions +
          '<div class="upload-zone" style="min-height:100px;cursor:pointer" onclick="var f=document.createElement(\'input\');f.type=\'file\';f.accept=\'image/*\';f.onchange=function(){var r=new FileReader();r.onload=function(ev){AdminEditor.__vizData[' + idx + '].data.url=ev.target.result;AdminEditor.renderAll()};r.readAsDataURL(this.files[0])};f.click()">' +
          (iu ? '<img src="' + iu + '" style="max-width:100%;max-height:200px;border-radius:4px">' : '<div class="icon">📁</div><p>Click to upload</p>') +
          '</div>' +
          '<input value="' + iu + '" placeholder="Image URL" onchange="AdminEditor.__vizData[' + idx + '].data.url=this.value;AdminEditor.renderAll()" style="width:100%;border:1px solid #eee;border-radius:3px;font-size:11px;padding:4px 8px;margin-top:4px">' +
          '<input value="' + (m.data.alt || '') + '" placeholder="ALT text" onchange="AdminEditor.__vizData[' + idx + '].data.alt=this.value" style="width:100%;border:1px solid #eee;border-radius:3px;font-size:11px;padding:4px 8px;margin-top:2px">' +
          '</div>';
        break;
      default:
